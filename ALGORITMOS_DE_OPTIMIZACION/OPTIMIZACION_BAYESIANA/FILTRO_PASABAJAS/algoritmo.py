...
# RUTAS DE EJECUCIÓN
# ============================================================
NGSPICE_EXE = r"C:\Users\Admin\Documents\ngSPICE\Spice64\bin\ngspice.exe"
...
def cargar_configuracion(ruta):
    ...
    cfg = {
        ...
        "n_iniciales": int(_buscar_etiqueta(parametros, "n_iniciales")),
        "n_iteraciones": int(_buscar_etiqueta(parametros, "n_iteraciones")),
        "xi": float(_buscar_etiqueta(parametros, "xi")),
        "n_candidatos": int(_buscar_etiqueta(parametros, "n_candidatos")),
        "n_restarts": int(_buscar_etiqueta(parametros, "n_restarts")),
        "semilla": int(_buscar_etiqueta(parametros, "semilla")),
    }
...
def guardar_resultado_json(mejor_global, mejor_fit, archivo_salida=ARCHIVO_RESULTADO_JSON,
                            archivo_grafica=GRAFICA_ARCHIVO, tiempo_ejecucion=None):
    """
    ...
    resultado = {
        "fitness": mejor_fit,
        "tiempo_ejecucion_s": tiempo_ejecucion,
...
# ============================================================
# OPTIMIZACION BAYESIANA (Proceso Gaussiano + Expected Improvement)
# ============================================================
# Cada componente R/C se representa como un indice CONTINUO dentro de su
# serie comercial (E12 para R, E6 para C): mismo espacio de busqueda que
# el AG/PSO (indices 0..len(serie)-1), pero la BO se mueve en el
# continuo y solo redondea al evaluar el circuito real, igual que hace
# PSO con sus particulas.
LIMITES = [(len(SERIE_E12) if c["tipo"] == "R" else len(SERIE_E6)) - 1 for c in COMPONENTES]
BOUNDS = np.array([[0.0, float(l)] for l in LIMITES])

def redondear_individuo(posicion):
    """Convierte un punto continuo de la BO al indice entero (discreto)
    de la serie comercial correspondiente, recortando a los limites."""
    return [int(round(min(max(p, 0.0), LIMITES[i]))) for i, p in enumerate(posicion)]

def punto_aleatorio():
    return np.array([random.uniform(0, LIMITES[i]) for i in range(NUM_PARAMETROS)])

def construir_gp():
    """Kernel Matern (nu=2.5) anisotropico + ruido blanco, uno por cada
    dimension (un R o C a optimizar). El Matern generaliza al RBF y se
    comporta mejor con superficies de fitness no perfectamente suaves."""
    kernel = (
...
def expected_improvement(X, gp, y_mejor, xi=None):
    """EI para MAXIMIZACION (el fitness de este proyecto es 0..1, mas
    alto = mejor, al reves que un error a minimizar)."""
    if xi is None:
        xi = XI
    ...
    z = mejora / sigma
    ei = mejora * norm.cdf(z) + sigma * norm.pdf(z)
    return ei

def proponer_siguiente_punto(gp, y_mejor):
    """Maximiza EI: muestreo aleatorio grueso sobre BOUNDS + refinamiento
    local con L-BFGS-B desde los mejores candidatos."""
...
def imprimir_encabezado():
    ...
    print(f"BO: puntos_iniciales={N_INICIALES} | iteraciones={N_ITERACIONES} | "
          f"xi={XI} | n_candidatos={N_CANDIDATOS} | n_restarts={N_RESTARTS}")
    if MODO == "BASICO":
        print(f"MODO BASICO (Fc + pendiente) | Fc objetivo={FC_OBJETIVO} Hz | "
              f"Pendiente objetivo={PENDIENTE_OBJETIVO_FC:.1f} dB/dec\n")
        encabezado = (f"{'Gen':<6} | {'Fit':<6} | {'Fc (Hz)':<10} | {'Amp (V)':<8} | {'Pend (dB/dec)':<15} | "
        ...
        encabezado = (f"{'Gen':<6} | {'Fit':<6} | {'Amp Paso (V)':<13} | {'Amp Aten (V)':<13} | {'Pend (dB/dec)':<13} | "
...
def ejecutar_BO():
    ...
    # Cache de fitness: evita volver a correr SPICE para un individuo ya
    ...
    imprimir_generacion(-1, mejor_fit, mejor_res)
    ...
    for it in range(N_ITERACIONES):
        ...
        imprimir_generacion(it, mejor_fit, mejor_res)
    ...
    print(f"Tiempo total: {tiempo_total:.3f} s | Evaluaciones SPICE únicas: {len(cache_fitness)}")
    ...
    graficar_resultado()
    guardar_resultado_json(mejor_global, mejor_fit, tiempo_ejecucion=tiempo_total)
