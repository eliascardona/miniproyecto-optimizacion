...
def obtener_metricas():
    try:
        ...
        # Medición de pendiente adaptativa (Cambio para ser universal)
        idx_fc = np.argmin(np.abs(frecuencias - fc_real))
        # Si es pasa-bajas, medimos a la derecha (x10), si es pasa-altas, a la izquierda (/10)
        f_ref = fc_real * 10.0 if TIPO_FILTRO == "PASABAJAS" else fc_real / 10.0
        idx_ref = np.argmin(np.abs(frecuencias - f_ref))
        
        v_fc = amplitudes[idx_fc] + 1e-12
        v_ref = amplitudes[idx_ref] + 1e-12
        
        pend = abs(20 * np.log10(v_fc) - 20 * np.log10(v_ref))
